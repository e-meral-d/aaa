...
class TemporalAdapterBlock(nn.Module):
    def __init__(self, embed_dim: int, num_heads: int = 8, mlp_ratio: float = 2.0, dropout: float = 0.0):
        super().__init__()
...
class TemporalAdapter(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                TemporalAdapterBlock(embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, dropout=dropout)
    ...
    def forward(self, tokens: torch.Tensor, batch_size: int, num_frames: int) -> torch.Tensor:
        ...
        for block in self.blocks:
            tokens = block(tokens)
        tokens = tokens.view(batch_size, seq_len, num_frames, embed_dim).permute(0, 2, 1, 3).contiguous()
        tokens = tokens.view(batch_total, seq_len, embed_dim)
        return tokens
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, design_details: dict = None):
        super().__init__()
        ...
        temporal_cfg = (design_details or {}).get("temporal_adapter_cfg", {}) if design_details else {}
        if temporal_cfg.get("enabled", False):
            self.temporal_adapter_enabled = True
            self.temporal_cfg = temporal_cfg
            self.temporal_adapter = TemporalAdapter(
                embed_dim=output_dim,
                num_layers=temporal_cfg.get("layers", 1),
                num_heads=temporal_cfg.get("num_heads", heads),
                mlp_ratio=temporal_cfg.get("mlp_ratio", 2.0),
                dropout=temporal_cfg.get("dropout", 0.0),
            )
        else:
            self.temporal_adapter_enabled = False
            self.temporal_cfg = {}
            self.temporal_adapter = None

    def _apply_temporal_adapter(self, tokens: torch.Tensor, temporal_info: dict):
        if not self.temporal_adapter_enabled or self.temporal_adapter is None or temporal_info is None:
            return tokens
        batch_size = temporal_info.get('batch_size')
        num_frames = temporal_info.get('num_frames', 1)
        if batch_size is None or num_frames is None or num_frames <= 1:
            return tokens
    ...
    @torch.no_grad()
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False, temporal_adapter=None):
        ...
        transformer_output, patch_tokens = self.transformer(x, features_list, DPAM_layer = DPAM_layer, ffn = ffn)
        if isinstance(transformer_output, list):
            x, x_ori = transformer_output
        else:
            x = transformer_output
            x_ori = transformer_output
        

        patch_token_list = []
        for patch_token in patch_tokens:
            patch_token = self.ln_post(patch_token.permute(1, 0, 2))
            patch_token = patch_token @ self.proj  # LND -> NLD
            patch_token = self._apply_temporal_adapter(patch_token, temporal_adapter)
            patch_token_list.append(patch_token)
        patch_tokens = patch_token_list

        global_tokens = x_ori[0, :, :] @ self.proj
        global_tokens = self._apply_temporal_adapter(global_tokens.unsqueeze(1), temporal_adapter).squeeze(1)
